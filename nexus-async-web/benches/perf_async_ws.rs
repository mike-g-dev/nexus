//! Async throughput benchmark: nexus-async-web vs tokio-tungstenite.
//!
//! Three benchmarks:
...
fn main() {
    ...
    rt.block_on(async {
        ...
        for &(size, label) in &[(40, "40B"), (128, "128B"), (512, "512B")] {
            ...
            report("nexus-async-web (recv)", e, c);
        ...
        for &(size, label) in &[(40, "40B"), (128, "128B")] {
            ...
            report("nexus-async-web (Stream)", e, c);
            let (e, c) = bench_inmemory_tungstenite(&wire, n).await;
        ...
        for &(size, label) in &[(40, "40B"), (128, "128B")] {
            ...
            report("nexus-async-web (tokio)", e, c);
        ...
        for &(size, label) in &[(40, "40B"), (128, "128B")] {
            ...
            report("nexus-async-web (tokio+TLS)", e, c);
