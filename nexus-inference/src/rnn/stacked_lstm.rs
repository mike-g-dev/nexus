...
///
/// Stacks N single-layer LSTMs where each layer's hidden state feeds
/// as input to the next. Output projection applies only to the final
/// layer's hidden state. Matches PyTorch's `nn.LSTM(num_layers=N)`.
///
...
/// let num_layers = 2;
///
/// // Layer 0: (4*8, 4) input weights, (4*8, 8) hidden weights
/// let wih_l0 = vec![0.1_f32; 4 * hidden_size * input_size];
/// let whh_l0 = vec![0.1_f32; 4 * hidden_size * hidden_size];
/// let bih_l0 = vec![0.0_f32; 4 * hidden_size];
/// let bhh_l0 = vec![0.0_f32; 4 * hidden_size];
///
/// // Layer 1: (4*8, 8) input weights (input = prev hidden), (4*8, 8) hidden weights
/// let wih_l1 = vec![0.1_f32; 4 * hidden_size * hidden_size];
/// let whh_l1 = vec![0.1_f32; 4 * hidden_size * hidden_size];
/// let bih_l1 = vec![0.0_f32; 4 * hidden_size];
/// let bhh_l1 = vec![0.0_f32; 4 * hidden_size];
///
/// // Output projection from final layer
...
impl LstmLayer {
    fn new(
        ...
        let concat_size = layer_input_size + hidden_size;
    ...
    fn step(&mut self, input: &[f32], hidden_size: usize) {
        let concat_size = input.len() + hidden_size;
        let gate_count = 4 * hidden_size;

        self.concat[..input.len()].copy_from_slice(input);
        self.concat[input.len()..concat_size].copy_from_slice(&self.h);
...
impl StackedLstm {
    ...
    /// `layers_bias_ih`, `layers_bias_hh` corresponds to one LSTM
    /// layer. The number of layers is determined by the slice lengths
    /// (must all be equal and >= 1).
    ///
    /// Layer 0 expects `weight_ih` shape `(4*hidden, input_size)`.
    /// Layers 1+ expect `weight_ih` shape `(4*hidden, hidden_size)`.
    /// All layers expect `weight_hh` shape `(4*hidden, hidden_size)`.
    ///
    ...
    ///
    /// Each layer processes the input through its LSTM gates, updating
    /// its own hidden and cell state. Layer 0 receives the user's input;
    ...
    /// Cell state of a specific layer.
    ///
    /// # Panics
    ///
    /// Panics if `layer >= n_layers()`.
...
mod tests {
    ...
    fn two_layer_differs_from_single() {
        let mut single = make_stacked_lstm(4, 8, 1, 1, 0.1);
    ...
    fn reset_clears_all_layers() {
        ...
        for k in 0..3 {
            ...
            assert!(lstm.cell_state(k).iter().all(|&v| v == 0.0));
        }
    }
